...
(async () => {
  ...
  contract = new Contract("localhost", () => {
    ...
    contract.credit((error, result) => {
      document.getElementById("creditAmount").innerText =
        "Available Credit For Withdrawal : " + result;
    });
    ...
    for (let a = 0; a < jsonResponse.length; a++) {
      ...
      flightnumber.appendChild(
        document.createTextNode(jsonResponse[a].flightNumber)
      );

      depart.appendChild(
        document.createTextNode(
          new Date(jsonResponse[a].timestamp).toString().substring(0, 28)
        )
      );
    ...
    DOM.elid("flight-number-list").onchange = function () {
      ...
      optionTime.text = new Date(jsonResponse[indexFlightTimestamp].timestamp)
        .toString()
        .substring(0, 28);
...
function purchaseFlight(error, flight) {
  if (error == null) {
    ...
    var departureTimeFormatted = new Date(parseInt(flight.timestamp))
      .toString()
      .substring(0, 28);
    depart.appendChild(document.createTextNode(departureTimeFormatted));

    // pass in insurance amount
    insureAmount.appendChild(
      document.createTextNode(flight.price / 1000000000000000000)
    );
